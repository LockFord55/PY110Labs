...
def add_to_wishlist(request, id_product: str) -> bool:
    """
    Добавляет продукт в избранное, если его там нет.
    ...
    if id_product not in wishlist["products"] and id_product in DATABASE:
...
def remove_from_wishlist(request, id_product: str) -> bool:
    """
    Если в избранном есть такой продукт, то удаляет его по значению.
